...
// Open chatbot by default on desktop
if (chatbotContainer && window.innerWidth > 600) {
    chatbotContainer.classList.add('open');
}

// ====== LLM Configuration ======
const CHATBOT_WORKER_URL = 'https://azizul-gemini-proxy.kaiyumx.workers.dev/';
let chatHistory = [];
// ====== End LLM Configuration ======

// ==========================================
// FALLBACK KNOWLEDGE BASE (Azizul Haque)
// ==========================================

const azizulKnowledge = {
    name: "Azizul Haque",
    ...
    about: "Azizul Haque is a Machine Learning Engineer and AI Researcher currently pursuing an M.Eng in Applied Artificial Intelligence at Stevens Institute of Technology.",

    experience: [
        {
            title: "AI Research Assistant – Stevens Institute of Technology",
            desc: "Working on LLM truthfulness evaluation using the TruthfulQA benchmark and automated evaluation pipelines."
        },
        {
            title: "Senior Web Developer – ExertPro LLC",
            desc: "Developing scalable web systems and integrating AI-driven features."
        },
        {
            title: "Data Analyst – NEXT Ventures",
            desc: "Performed SQL-based data analysis and developed dashboards using Python."
        }
    ],

    education: [
        "M.Eng in Applied Artificial Intelligence – Stevens Institute of Technology",
        "B.Sc in Software Engineering – City University"
    ],

    research: [
        "LLM Truthfulness Evaluation (TruthfulQA)",
        "Depression Detection using MentalBERT",
        "Accessible AI Systems (ACM ASSETS 2025)",
    ...
    skills: [
        ...
        "Hugging Face Transformers", "DistilBERT", "LSTM",
        "React", "FastAPI", "Flask",
        "SQL", "Pandas", "NumPy"
    ]
};
...
const chatPatterns = [
    {
        ...
        response: () => `Hello! 👋 I'm here to share information about ${azizulKnowledge.name}. What would you like to know?`
    },
    {
        patterns: [/who (is|are you)|about/i],
        response: () => azizulKnowledge.about
    },
    {
        patterns: [/experience|work|career/i],
        response: () => {
            let resp = "Here is Azizul's professional experience:\n\n";
            azizulKnowledge.experience.forEach(exp => {
                resp += `🔹 **${exp.title}**\n${exp.desc}\n\n`;
            });
            return resp;
        }
    ...
    {
        patterns: [/education|degree|study/i],
        response: () => `🎓 **Education:**\n\n• ${azizulKnowledge.education.join('\n• ')}`
    },
    {
        patterns: [/research|interest/i],
        response: () => `🔬 **Research Areas:**\n\n• ${azizulKnowledge.research.join('\n• ')}`
    },
    {
        patterns: [/skill|technology|stack/i],
        response: () => `💻 **Technical Skills:**\n\n${azizulKnowledge.skills.join(', ')}`
    },
    {
        patterns: [/contact|email|reach|connect/i],
        response: () => `📬 **Contact Information:**\n\n📧 Email: ${azizulKnowledge.email}\n💼 LinkedIn: ${azizulKnowledge.linkedin}\n🐙 GitHub: ${azizulKnowledge.github}`
    }
];

const defaultResponses = [
    "I don't have specific information about that. You can ask about experience, research, education, or contact details.",
    "I'm not sure about that topic. Feel free to connect directly via LinkedIn or email."
];
...
async function sendMessage() {
    ...
    if (CHATBOT_WORKER_URL && CHATBOT_WORKER_URL !== 'PASTE_YOUR_WORKER_URL_HERE') {
        try {
            ...
            console.log('Worker unavailable. Using fallback.');
        }
    }
